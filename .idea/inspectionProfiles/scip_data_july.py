...
def create_compatible_dataframe(df):
    name_mapping_with_more_scip_features = {'#IntViols': 'nintpseudocost',
                                            ...
                                            'EqCons': '(nlinearequconss + nnonlinearequconss)',
                                            'NonlinCons': 'nnonlinearconss',
...
def read_in_and_call_process(directory_path_raw_data: str, saving_directory:str, to_csv=True):

    stefans_data_merged = process_directory_both_rules(directory_path_raw_data)
    ...
    stefans_data_merged_all_have_features.loc[:, ['Status Mixed', 'Status Int']] = (
        stefans_data_merged_all_have_features.loc[:, ['Status Mixed', 'Status Int']].replace('gap limit', 'optimal'))
    if to_csv:
        # save only df where all instances without features are deleted
        stefans_data_merged_all_have_features.to_csv(f'{saving_directory}/scip_default_raw.csv',
                               index=False)
        ...
        stefan_data_reduced_cols_no_nan.to_csv(
            f"{saving_directory}/scip_default_named_columns.csv",
            index=False)
        ...
        scip_features_df.to_csv(
            f"{saving_directory}/scip_default_features_logged.csv",
            index=False)
