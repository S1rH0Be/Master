...
def read_in_data():
    ...
    # features
    feats = pd.read_excel('/Users/fritz/Downloads/ZIB/Master/CSV/SecondIteration/Jetzt Ernst/CSVs/Master_Excel/features_pointfive_is_zero_df.xlsx')
...
def visualizer(data : pd.DataFrame, title : str, y_lims : List[float], x_ticks : List[str], sort_data=False, colors=None, colors_by_goodness=False, title_size=12):
...
def compare_sgm_relative_to_mixed(time_df : pd.DataFrame, shift : float):
    ...
    shifted_means_relative_to_mixed = [np.round(shifted_geometric_mean(time_df[col], shift)/shifted_mean_mixed, 2) for col in time_df.columns]
    shifted_means_df = pd.DataFrame({'Rule': time_df.columns, 'Shifted Geometric Mean': shifted_means_relative_to_mixed})

    visualizer(shifted_means_df, 'Shifted Geometric Means of RunTime of BranchRule relative to Mixed', [0, max(shifted_means_relative_to_mixed) * 1.1], ['Mixed', 'Int', 'Best', 'Predicted'], sort_data=True, colors=['green', 'magenta'])
...
class Regressor:
    ...
    def __init__(self, model : Union[LinearRegression(), RandomForestRegressor()], imputation_name : str, train_features : pd.DataFrame, test_features : pd.DataFrame, train_label : pd.Series, test_label : pd.Series, scaler : Union[Literal['byHand'], PowerTransformer, StandardScaler, None], time_data : pd.DataFrame):
        self.model = model
        ...
        self.prediction_df = pd.DataFrame({'Prediction': self.prediction, 'Actual': self.test_label.loc[self.relevant_indices]},
    ...
    def scale(self, feat_df : pd.DataFrame):
        if self.scaler is None:
            ...
            return pd.DataFrame(columns=feat_df.columns,
                                           data=yeo_john, index=feat_df.index)
    ...
    def visualize_feature_importance(score_df : pd.DataFrame):
        visualizer(score_df, 'Importance of Features', [min(score_df['Score']) * 1.1, max(score_df['Score']) * 1.1], score_df['Feature'], sort_data=False)

    def accuracy(self, title: str):
        self.prediction_df['Right or Wrong'] = (np.sign(self.prediction_df['Prediction']) == np.sign(self.prediction_df['Actual'])).astype(int)
        ...
        acc_df = pd.DataFrame({'Intervall': ['Complete', '[1,1.5)', '[1.5, inf)'], 'Accuracy': [total_accuracy, mid_accuracy, extreme_accuracy]})
...
def predict_all_combinations(regressoren, imputations, scalers, feature_space, label, t_df, compare_sgm=False, show_accuracy=True, show_feature_histogram=False):
        ...
        for imp in imputations:
            for scal in scalers:
                ...
                for feat in feature_space:
                    feats = FeaturesAndLabel(feat[0], label, 'X and y') #watch out need to change feature space as well
                    if imp == 'Median':
                        model = Regressor(regressor, imp, feats.feat_train_median_imputed, feats.feat_test_median_imputed,
                                                               feats.label_train, feats.label_test, scal, t_df)
                        if compare_sgm:
                            compare_sgm_relative_to_mixed(model.time_data, 0.5)
                        if show_accuracy:
                            accuracy_df[str(regressor)+" "+imp+" "+str(scal)+" "+feat[1]] = model.accuracy(str(regressor)+imp+str(scal)+feat[1])['Accuracy']
                        if show_feature_histogram:
                            model.visualize_features(bins=15)

                        if model.feature_importance['Score'].max() > 0:
                            importance_df[str(regressor) + imp + str(scal) + feat[1]] = model.feature_importance['Score']
...
def main():
    ...
    feature_space_filtered = [(x_filtered,'All'), (x_filtered[top_8_global], 'Glob8'), (x_filtered[top_8_lin], 'Lin8'), (x_filtered[top_8_for], 'For8')]
    feature_space_unfiltered = [(x, 'All'), (x[top_8_global], 'Glob8'), (x[top_8_lin], 'Lin8'), (x[top_8_for], 'For8')]

    acc_df, importance_df = predict_all_combinations(regressoren, imputations, scalers, feature_space_unfiltered, y, time, show_accuracy=True)
