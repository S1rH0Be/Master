...
from Pys.feature_distribution import scale_by_hand
...
def forest_regression(features:DataFrame, label:DataFrame, seed, title_idea:str,  show_accuracy=False):
    ...
    return pred_df, accuracy_df, mse, r2, feature_importance_df
...
def main(rand_seeds, acc_to_ex=False, sgm_to_excel=False):
    ...
    scaler = ['NoScaling', 'byHand', 'Yeo-Johnson']
                ...
                if scaling == 'NoScaling':
                    ...
                    for regressor in regressors:
                        if regressor == 'LinearRegression':
                            p, a, m, r, importance = linear_regression(imputed_data, label_series, rand_seed, title_idea=str(imputation)+scaling+regressor, show_accuracy=True)
                            ...
                            p, a, m, r, importance = forest_regression(imputed_data, label_series, rand_seed,  title_idea=str(imputation)+scaling+regressor, show_accuracy=True)
                            mean_to_mixed = predicted_time(time_mixed_int_vbs, p)
                            mean_to_mixed.extend(a['Accuracy'].values)
                            count += 1
                            columns_for_collected_sgm[str(count)] = mean_to_mixed
                    ...
                    for regressor in regressors:
                        if regressor == 'LinearRegression':
                            p, a, m, r, importance = linear_regression(scaled_features, label_series, rand_seed, title_idea=str(imputation)+scaling+regressor, show_accuracy=True)
                            mean_to_mixed = predicted_time(time_mixed_int_vbs, p)
                            mean_to_mixed.extend(a['Accuracy'].values)
                            count += 1
                            columns_for_collected_sgm[str(count)] = mean_to_mixed
                            ...
                            p, a, m, r, importance = forest_regression(scaled_features, label_series, rand_seed, title_idea=str(imputation)+scaling+regressor, show_accuracy=True)
                            mean_to_mixed = predicted_time(time_mixed_int_vbs, p)
                            mean_to_mixed.extend(a['Accuracy'].values)
                            count += 1
                            columns_for_collected_sgm[str(count)] = mean_to_mixed
...
def get_importances(importance_dict, to_excel=False):
