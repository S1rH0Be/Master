...
def read_in_and_call_process(data_set: str, random_noise_experiment=False, fico=True, to_csv=True):
    ...
    if not random_noise_experiment:
        stefans_data_merged = process_directory_both_rules(directory_path)
    else:
        stefans_data_merged = process_directory_random_noise(directory_path)
